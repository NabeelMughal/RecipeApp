              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                  {(recipe.gallery ?? []).map((img) => (
                      <div key={img.public_id} className="relative group aspect-square">
                          <img src={img.url} alt="Gallery image" title="Click to view larger" className="w-full h-full object-cover rounded-lg cursor-pointer border-2 border-transparent hover:border-[#c97c54]" onClick={() => setSelectedImage(img.url)} />
                          <div className="absolute top-1 right-1 flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button disabled={isSubmitting} onClick={() => updateRecipe({ action: 'delete-gallery-image', image: img })} className="bg-red-600/80 text-white rounded-full p-1 disabled:opacity-50" title="Delete image">
                                  <X size={14} />
                              </button>
                              <button disabled={isSubmitting} onClick={() => updateRecipe({ action: 'set-main-image', image: img })} className="bg-blue-600/80 text-white rounded-full p-1 disabled:opacity-50" title="Set as main image">
                                  <Pin size={14} />
                              </button>
                          </div>
                      </div>
        ...
        {selectedImage && (
          ...
          >
            ...
            <motion.div 
              ...
              className="relative"
            >
              <img src={selectedImage} alt="Enlarged recipe view" className="max-w-[90vw] max-h-[90vh] object-contain rounded-lg" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
